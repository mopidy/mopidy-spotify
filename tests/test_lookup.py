...
from mopidy_spotify import lookup, web
...
def test_lookup_of_invalid_your_uri(
    provider: SpotifyLibraryProvider, caplog: pytest.LogCaptureFixture
):
    results = provider.lookup_many([Uri("spotify:your:artists")])
...
def test_lookup_when_offline(
    ...
    results = provider.lookup_many([Uri("spotify:invalid:something")])
...
def test_lookup_of_track_uri(
    ...
    web_client_mock.get_batch.assert_called_once()
    assert len(results) == 1
    track = results[Uri("spotify:track:abc")][0]
...
def test_lookup_of_album_uri(
    ...
    web_album_mock_link: web.WebLink,
    provider: SpotifyLibraryProvider,
):
    web_client_mock.get_batch.return_value = [(web_album_mock_link, web_album_mock)]

    results = provider.lookup_many([Uri("spotify:album:def")])

    web_client_mock.get_batch.assert_called_once()
    assert len(results) == 1
    assert len(results[Uri("spotify:album:def")]) == 10
    track = results[Uri("spotify:album:def")][0]
    assert track.uri == "spotify:track:abc"
    assert track.name == "ABC 123"
    assert track.bitrate == 160
    assert track.album.name == "DEF 456"
...
def test_lookup_of_album_uri_empty_response(
    ...
    web_client_mock.get_batch.assert_called_once()
    assert len(results) == 1
    assert len(results[Uri("spotify:album:def")]) == 0
...
def test_lookup_of_artist_uri(
    ...
    results = provider.lookup_many([Uri("spotify:artist:abba")])

    assert len(results) == 1
    assert len(results[Uri("spotify:artist:abba")]) == 13

    track = results[Uri("spotify:artist:abba")][0]
    assert track.uri == "spotify:track:abc"
    assert track.name == "ABC 123"
    assert track.album.name == "DEF 456"
    assert track.bitrate == 160

    track = results[Uri("spotify:artist:abba")][10]
    assert track.uri == "spotify:track:abc"
...
def test_lookup_of_artist_ignores_unavailable_albums(
    ...
    assert len(results) == 1
    assert len(results[Uri("spotify:artist:abba")]) == 2
...
def test_lookup_of_artist_uri_ignores_compilations(
    ...
    assert len(results) == 1
    assert len(results[Uri("spotify:artist:abba")]) == 0
...
def test_lookup_of_playlist_uri(
    ...
    playlist_uri = Uri(web_playlist_mock["uri"])
...
def test_lookup_of_yourtracks_uri(
    web_client_mock: mock.MagicMock,
    web_track_mock: dict[str, Any],
    provider: SpotifyLibraryProvider,
):
    ...
    results = provider.lookup_many([Uri("spotify:your:tracks")])

    web_client_mock.get_all.assert_called_once()
    assert len(results) == 1
    assert len(results[Uri("spotify:your:tracks")]) == 4
    for track in results[Uri("spotify:your:tracks")]:
        assert track.uri == "spotify:track:abc"
        assert track.name == "ABC 123"
        assert track.bitrate == 160
        assert track.album.name == "DEF 456"

    assert results == provider.lookup_many([Uri("spotify:your:tracks")])
    assert web_client_mock.get_all.call_count == 2  # Not cached

    results = provider.lookup_many([Uri("spotify:track:abc")])
    assert len(results[Uri("spotify:track:abc")]) == 1
...
def test_lookup_of_youralbums_uri(
    ...
    results = provider.lookup_many([Uri("spotify:your:albums")])

    web_client_mock.get_all.assert_called_once()
    web_client_mock.get_batch.assert_called_once()
    assert len(results) == 1
    assert len(results[Uri("spotify:your:albums")]) == 4 * 10
    for track in results[Uri("spotify:your:albums")]:
        assert track.uri == "spotify:track:abc"
        assert track.name == "ABC 123"
    ...
    assert results == provider.lookup_many([Uri("spotify:your:albums")])
    assert web_client_mock.get_all.call_count == 2  # Not cached
...
def test_lookup_caches_tracks_albums(
    ...
    results1 = provider.lookup_many([Uri("spotify:album:def")])
    results2 = provider.lookup_many([Uri("spotify:album:def")])
    results3 = provider.lookup_many([Uri("spotify:track:abc")])

    web_client_mock.get_batch.assert_called_once()
    assert len(results1) == 1
    assert len(results1[Uri("spotify:album:def")]) == 10
    track = results1[Uri("spotify:album:def")][0]
    assert track.uri == "spotify:track:abc"

    assert len(results2) == 1
    assert len(results2[Uri("spotify:album:def")]) == 10
    track = results2[Uri("spotify:album:def")][0]
    assert track.uri == "spotify:track:abc"

    assert len(results3) == 1
    assert len(results3[Uri("spotify:track:abc")]) == 1
    track = results3[Uri("spotify:track:abc")][0]
    assert track.uri == "spotify:track:abc"


def test_lookup_caches_no_cache_playlist(
    web_client_mock: mock.MagicMock,
    web_playlist_mock: dict[str, Any],
    provider: SpotifyLibraryProvider,
):
    ...
    assert len(results3) == 1
    assert results3[Uri("spotify:track:abc")][0].uri == "spotify:track:abc"


def test_lookup_no_cache_artist(
    web_client_mock: mock.MagicMock,
    web_album_mock: dict[str, Any],
    provider: SpotifyLibraryProvider,
):
    ...
    artist_uri = Uri("spotify:artist:abba")
    results1 = provider.lookup_many([artist_uri])
    results2 = provider.lookup_many([artist_uri])
    results3 = provider.lookup_many(
        [Uri("spotify:album:def"), Uri("spotify:track:abc")]
    )
    ...
    assert results3[Uri("spotify:album:def")][0].uri == "spotify:track:abc"
    assert results3[Uri("spotify:track:abc")][0].uri == "spotify:track:abc"
